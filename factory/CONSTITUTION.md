...
1. **Public launch.** GitHub About, topics, and homepage are set. README describes the product and how to start a desk. `assert-stealth.sh` checks that those surfaces stay filled. The job stays a `needs:` of aggregator `CI`.
...
8. **No auto-merge** for release, publish, constitution amend, stealth-script PRs, diffs that touch `.github/workflows/`, or `factory/scripts/ci-pytest-shard.py`.
