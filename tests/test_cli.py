...
def test_run_unimplemented_catalog_lang_exits(monkeypatch, tmp_path, capsys) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "missing.json"))
...
def test_run_unknown_lang_id_exits(monkeypatch, tmp_path, capsys) -> None:
    ...
    capsys.readouterr()
    code = main(["run", "bank_system", "--lang", "notalang", "--level", "1", "--kind", "solution"])
    captured = capsys.readouterr()
    out = captured.out + captured.err
    assert code == 1
...
def test_start_picker_lists_problem_level_counts(monkeypatch, tmp_path, capsys) -> None:
    ...
    assert "load_balancer (4 levels)" in out
    assert "pubsub (4 levels)" in out
    assert "text_editor (4 levels)" in out
