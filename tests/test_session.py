...
def test_submit_unlocks_when_workspace_write_fails(monkeypatch, tmp_path: Path, capsys) -> None:
    ...
    assert workspace_note_next() in out
...
def test_console_workspace_write_fail_prints_next(monkeypatch, tmp_path: Path, capsys) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
    assert main(["start", "bank_system", "python3", "--reset", "--no-console"]) == 0
    capsys.readouterr()
    ...
    monkeypatch.setattr("honepad.console.refresh_workspace", boom)
    monkeypatch.setattr("honepad.console.loop_console", lambda *_a, **_k: 0)
    code = main(["console"])
    out = capsys.readouterr().out
    assert code == 0
    assert "NOTE:" in out
...
def test_submit_last_rate_limiter_prints_next_gpu_scheduler(
    monkeypatch, tmp_path: Path, capsys
) -> None:
    session_file = tmp_path / "session.json"
    monkeypatch.setenv("HONEPAD_SESSION", str(session_file))
    ...
    assert main(["submit", "rate_limiter", "--kind", "solution", "--confirm", "y"]) == 0
    out = capsys.readouterr().out
    assert "DONE: rate_limiter python3" in out
    assert "NEXT: " in out
...
@pytest.mark.skipif(shutil.which("ruby") is None, reason="ruby not found")
def test_submit_rejects_ruby_exact_count_fake_json_exit(
    monkeypatch, tmp_path: Path, capsys
) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
    assert main(["start", "bank_system", "ruby", "--reset", "--no-console"]) == 0
    ...
    work.write_text(
        "class Simulation\n"
        "  def initialize\n"
        f'    $stdout.write({payload!r} + "\\n")\n'
        "    exit 0\n"
        "  end\n"
        "end\n",
        encoding="utf-8",
    )
    code = main(["submit", "bank_system", "--lang", "ruby"])
    captured = capsys.readouterr()
    out = captured.out + captured.err
    assert code == 1
    assert "FAIL" in out
...
@pytest.mark.skipif(shutil.which("node") is None, reason="node not found")
def test_submit_rejects_javascript_exact_count_fake_json_write_sync(
    monkeypatch, tmp_path: Path, capsys
) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
    assert main(["start", "bank_system", "javascript", "--reset", "--no-console"]) == 0
    ...
    work.write_text(
        ...
        "  constructor() {\n"
        "    const fs = require('fs');\n"
        f"    fs.writeSync(1, {payload + chr(10)!r});\n"
        "    process.exit(0);\n"
        "  }\n"
        "  createAccount() { return true; }\n"
        "}\n"
...
@pytest.mark.skipif(shutil.which("php") is None, reason="php not found")
def test_submit_rejects_php_exact_count_fake_json_exit(monkeypatch, tmp_path: Path, capsys) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
    assert main(["start", "bank_system", "php", "--reset", "--no-console"]) == 0
    capsys.readouterr()
    work = tmp_path / "work" / "bank_system" / "php" / "work.php"
    payload = _exact_l1_pass_json()
    work.write_text(
        "<?php\n"
        "class Simulation {\n"
        "  public function __construct() {\n"
        f"    echo {json.dumps(payload + chr(10))};\n"
        "    exit(0);\n"
        "  }\n"
        "}\n",
        encoding="utf-8",
    )
    code = main(["submit", "bank_system", "--lang", "php"])
    captured = capsys.readouterr()
    out = captured.out + captured.err
    assert code == 1
    assert "FAIL" in out
...
@pytest.mark.skipif(shutil.which("Rscript") is None, reason="Rscript not found")
def test_submit_rejects_r_exact_count_fake_json_exit(monkeypatch, tmp_path: Path, capsys) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
    assert main(["start", "bank_system", "r", "--reset", "--no-console"]) == 0
    capsys.readouterr()
    work = tmp_path / "work" / "bank_system" / "r" / "work.R"
    payload = _exact_l1_pass_json()
    work.write_text(
        f'Simulation <- function() {{\n  cat({payload!r}, "\\n", sep="")\n  quit(status=0)\n}}\n',
        encoding="utf-8",
    )
    code = main(["submit", "bank_system", "--lang", "r"])
    captured = capsys.readouterr()
    out = captured.out + captured.err
    assert code == 1
    assert "FAIL" in out
...
@pytest.mark.skipif(shutil.which("groovy") is None, reason="groovy not found")
def test_submit_rejects_groovy_exact_count_fake_json_exit(
    monkeypatch, tmp_path: Path, capsys
) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
    assert main(["start", "bank_system", "groovy", "--reset", "--no-console"]) == 0
    capsys.readouterr()
    work = tmp_path / "work" / "bank_system" / "groovy" / "work.groovy"
    payload = _exact_l1_pass_json()
    work.write_text(
        "class Simulation {\n"
        "  Simulation() {\n"
    ...
    code = main(["submit", "bank_system", "--lang", "groovy"])
    captured = capsys.readouterr()
    out = captured.out + captured.err
    assert code == 1
    assert "FAIL" in out
...
def test_submit_rejects_go_exact_count_fake_json_exit(monkeypatch, tmp_path: Path, capsys) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
...
def test_submit_rejects_go_init_planted_report_json_exit(
    ...
    work.write_text(
        ...
        ")\n\n"
        "func init() {\n"
        f'\t_ = os.WriteFile("report.json", []byte(`{payload}`), 0644)\n'
        "\tos.Exit(0)\n"
        "}\n\n"
        "type Simulation struct{}\n\n"
        "func NewSimulation() *Simulation { return &Simulation{} }\n",
...
def test_submit_rejects_csharp_exact_count_fake_json_exit(
    ...
    assert main(["start", "bank_system", "csharp", "--reset", "--no-console"]) == 0
    capsys.readouterr()
    work = tmp_path / "work" / "bank_system" / "csharp" / "work.cs"
    payload = _exact_l1_pass_json()
    work.write_text(
        "public class Simulation\n"
        "{\n"
        "    public Simulation()\n"
        "    {\n"
        f"        System.Console.WriteLine({json.dumps(payload)});\n"
        "        System.Environment.Exit(0);\n"
        "    }\n"
        "}\n",
        encoding="utf-8",
    )
    code = main(["submit", "bank_system", "--lang", "csharp"])
    captured = capsys.readouterr()
    out = captured.out + captured.err
    assert code == 1
    assert "FAIL" in out
...
@pytest.mark.skipif(shutil.which("dotnet") is None, reason="dotnet not found")
def test_submit_rejects_csharp_openstandardoutput_fake_json_exit(
    monkeypatch, tmp_path: Path, capsys
) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
    assert main(["start", "bank_system", "csharp", "--reset", "--no-console"]) == 0
    ...
    work.write_text(
        ...
        "    public Simulation()\n"
        "    {\n"
        f"        string payload = {json.dumps(payload)};\n"
...
def test_submit_rejects_cpp_exact_count_fake_json_exit(monkeypatch, tmp_path: Path, capsys) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
...
@pytest.mark.skipif(shutil.which("c++") is None, reason="c++ not found")
def test_submit_rejects_cpp_ctor100_fake_json_exit(monkeypatch, tmp_path: Path, capsys) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
    assert main(["start", "bank_system", "cpp", "--reset", "--no-console"]) == 0
    capsys.readouterr()
    work = tmp_path / "work" / "bank_system" / "cpp" / "work.cpp"
    ...
    work.write_text(
        ...
        "  }\n"
        "};\n"
        "__attribute__((constructor(100))) static void fake_pass() {\n"
        f"  std::cout << {json.dumps(payload)} << std::endl;\n"
        "  std::exit(0);\n"
        "}\n"
        "#endif\n",
...
@pytest.mark.skipif(shutil.which("javac") is None, reason="javac not found")
def test_submit_rejects_java_exact_count_fake_json_fd_exit(
    monkeypatch, tmp_path: Path, capsys
) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
    assert main(["start", "bank_system", "java", "--reset", "--no-console"]) == 0
    ...
    work.write_text(
        ...
        f"        out.println({json.dumps(payload)});\n"
        "        System.exit(0);\n"
        "    }\n"
        "}\n",
        encoding="utf-8",
    )
    code = main(["submit", "bank_system", "--lang", "java"])
    captured = capsys.readouterr()
    out = captured.out + captured.err
    assert code == 1
    assert "FAIL" in out
...
@pytest.mark.skipif(shutil.which("javac") is None, reason="javac not found")
def test_submit_rejects_java_static_init_fake_json_fd_exit(
    monkeypatch, tmp_path: Path, capsys
) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
    assert main(["start", "bank_system", "java", "--reset", "--no-console"]) == 0
...
@pytest.mark.skipif(shutil.which("kotlinc") is None, reason="kotlinc not found")
def test_submit_rejects_kotlin_exact_count_fake_json_exit(
    monkeypatch, tmp_path: Path, capsys
) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
    assert main(["start", "bank_system", "kotlin", "--reset", "--no-console"]) == 0
    capsys.readouterr()
    work = tmp_path / "work" / "bank_system" / "kotlin" / "work.kt"
    payload = _exact_l1_pass_json()
    work.write_text(
        "class Simulation {\n"
        "    init {\n"
    ...
    code = main(["submit", "bank_system", "--lang", "kotlin"])
    captured = capsys.readouterr()
    out = captured.out + captured.err
    assert code == 1
    assert "FAIL" in out
...
@pytest.mark.skipif(shutil.which("cargo") is None, reason="cargo not found")
def test_submit_rejects_rust_exact_count_fake_json_exit(
    monkeypatch, tmp_path: Path, capsys
) -> None:
    monkeypatch.setenv("HONEPAD_SESSION", str(tmp_path / "session.json"))
    assert main(["start", "bank_system", "rust", "--reset", "--no-console"]) == 0
    capsys.readouterr()
    work = tmp_path / "work" / "bank_system" / "rust" / "work.rs"
    payload = _exact_l1_pass_json()
    work.write_text(
        ...
        "impl Simulation {\n"
        "    pub fn new() -> Self {\n"
        f'        let _ = writeln!(io::stdout(), "{{}}", r#"{payload}"#);\n'
        "        std::process::exit(0);\n"
        "    }\n"
        "}\n\n"
    ...
    code = main(["submit", "bank_system", "--lang", "rust"])
    captured = capsys.readouterr()
    out = captured.out + captured.err
    assert code == 1
    assert "FAIL" in out
